...
import CustomAutoComplete, { type CustomAutoCompleteOptions } from "../components/common-components/custom-auto-complete/custom-auto-complete";
...
const AssignPatient = ({ onClose }: { onClose?: () => void }) => {
    ...
    const onSubmit: SubmitHandler<PatientForm> = () => {
        ...
        console.log("SELECTED PATIENT IDs:", selectedPatients);

        const fullPatientDetails = allPatients.filter((p) =>
            selectedPatients.includes(p.key)
        );

        console.log("SELECTED PATIENT DETAILS:", fullPatientDetails);
    };
    ...
    return (
        <DrawerBody padding="16px 20px" offset={footerRef?.current?.offsetHeight} gap={1}>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Box mt={'6px'} sx={(theme) => ({ border: '1px solid', borderColor: theme.palette.neutral[5], p: '1rem', gap: '1rem', borderRadius: '6px' })}>
                    <CustomCheckBox
